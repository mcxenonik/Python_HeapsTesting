...
class Heap4:
    ...
    def _check(self, value):
        ...
        if ((child_index + 2) % 4 == 0) or ((child_index + 1) % 4 == 0):
            parent_index = ceil(child_index / 4)
        elif (child_index - 1) % 4 == 0:
            parent_index = floor(child_index / 4)
        elif child_index % 4 == 0:
            parent_index = child_index // 4
    ...
    def display(self):
        ...
        while line_pointer <= len(self.heap):
            ...
            if line_pointer * 4 > len(self.heap):
                for element in range(line_pointer, len(self.heap)):
                    print(self.heap[element], end = " ")
            line_pointer = line_pointer * 4 - 2